...
from nn.scalegmn.graph_init import GraphInit
from nn.scalegmn.utils import graph_to_wb
from nn.scalegmn.layers import DeepSet, PermScaleInvariantReadout, MLPNet, PositionalEncoding, EquivariantNet, EdgeUpdate
from nn.scalegmn.gnn_layers import ScaleEq_GNN_layer, GNN_layer, ScaleGMN_GNN_layer_aggr, GNN_layer_aggr
from nn.scalegmn.mlp import mlp
from experiments.data.scalegmn_data_utils import get_node_types, get_edge_types
...
class ScaleGMN_equiv(BaseScaleGMN):
    ...
    def __init__(self, model_args, **kwargs):
        ...
        # todo: consider removing since we have this in the training script
...
class ScaleGMN_GNN_bidir(ScaleGMN_GNN):
    ...
    def __init__(self, model_args, layer_idx):
        ...
        bw_layers = []
        upd_layers = []
        self.equiv_out_features = model_args.get("equiv_out_features", 1)
        ...
        for i in range(model_args['num_layers']):
            ...
            if model_args['symmetry'] != 'permutation':
                upd_layers.append(EquivariantNet(
                            ...
                            3*self.d_hid,
                            self.equiv_out_features if self.equivariant and last_layer else self.d_hid,
                            model_args['mlp_args'],
            ...
            if self.update_edge_attr and (i < model_args['num_layers'] - 1 or self.equivariant):
                if model_args['symmetry'] != 'permutation':
                    fw_upd_edge_layers.append(EdgeUpdate(
                        d_in_v,
                        self.equiv_out_features if self.equivariant and last_layer else d_in_e,
                        model_args['mlp_args'],
                        model_args['symmetry'],
                        sign_symmetrization=model_args['gnn_args']['sign_symmetrization']))
