...
from nn.dws.layers import BN, DownSampleDWSLayer, Dropout, DWSLayer, InvariantLayer, ReLU
...
class DWSModel(nn.Module):
    def __init__(
        ...
        if input_dim_downsample is None:
            layers = [
                BN(input_features, len(weight_shapes), len(bias_shapes)),  # todo: remove after test!!!!
                DWSLayer(
